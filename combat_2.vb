Public Class combat_2
    ...
    Private Sub combat_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Tag Is "Direct Fire" Or Tag Is "Air to Air" Then return_fire.Visible = True Else return_fire.Visible = False
        If Tag Is "Smoke Barrage" Then
        ...
        End If
        If Tag Is "Indirect Fire" Then
            fire.Visible = True
        End If
        If Tag Is "Direct Fire" Or Tag Is "Opportunity Fire" Then
            obs_chart.Show()
            directfirepanel.Visible = True
            indirectfirepanel.Visible = False
        End If
        If Tag Is "Opportunity Fire" Then swap.Visible = False
        If Me.Tag Is "Air Ground" Or Me.Tag Is "SEAD" Then
        ...
        ElseIf Me.Tag Is "CAP" Or Me.Tag Is "Intercept" Then
            targets.Visible = True
    ...
    Private Sub choose_weapon(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles fwpn.Click, twpn.Click
        If Tag Is "Smoke Barrage" Then Exit Sub
        Dim t As String = ""
    ...
    Public Sub firer_strength(f1 As Object, f2 As Object, f3 As Object, strength As Integer, airunit As Boolean)
        ...
        If strength <= 0 Then
            ...
            f3.Enabled = False
            Exit Sub
        ElseIf strength <= 5 Or Tag Is "Indirect Fire" Or airunit Then
            f1.Enabled = True
            f2.Enabled = False
            f3.Enabled = False
        ...
        If Tag Is "Ground to Air" And (Not firer.missile_armed Or (firer.missile_armed And firer.missiles > 0)) Then firer.firers_available = firer.strength : strength = firer.firers_available
        If Tag Is "Indirect Fire" Then strength = -1
        ...
        End Select
        If Tag Is "Air to Air" Or Tag Is "Air to Ground" Then
        ...
        ElseIf Strings.Left(f1.name, 1) = "t" Then
            target.firers = 0
        ElseIf Tag Is "Smoke Barrage" Then
            f1.Enabled = False
            f2.Enabled = False
            f3.Enabled = False
        Else
    ...
    Public Sub update_parameters(opt As String)
        If opt = "firers" Or opt = "artillery" Then
            If Tag Is "Indirect Fire" Then reset_unit_options(indirectfirepanel) Else reset_unit_options(directfirepanel)
            ...
            If firer.debussed_gt = 0 Or firer.debussed_gt = phase Then
                ...
                End If
            End If
            If Tag Is "Indirect Fire" Or Tag Is "Smoke Barrage" Then
        ...
        ElseIf opt = "targets" Then
            ...
            If target.w2 <> "" And Tag Is "Direct Fire" Then twpn.Enabled = True Else twpn.Enabled = False
    ...
    Private Sub select_units(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles targets.Click, firers.Click, observers.Click, artillery.Click
        ...
        If sender.name = "targets" Then
            ...
            Else
                ...
                If sender.FocusedItem.Text = "No Effect" And Tag Is "Indirect Fire" Then
                    target = New cunit
        ...
        ElseIf sender.name = "artillery" Then
            ...
            ElseIf sender.focuseditem.backcolor <> golden And ph_units(sender.FocusedItem.Text).valid_arty_observer(observer) Then
                If Tag Is "Indirect Fire" And Not firer.title Is Nothing Then If Not artillery.FindItemWithText(firer.title) Is Nothing Then artillery.FindItemWithText(firer.title).BackColor = firer.status("")
                sender.focuseditem.backcolor = golden
                firer = ph_units(sender.FocusedItem.Text)
                If Tag Is "Indirect Fire" Then firer_strength(a1, a2, a3, firer.firers_available, firer.airborne)
            Else
    ...
    'If Tag is "Indirect Fire" And firer.role = "|RL|" Then
    '    area_fire = True
    'Else
    ...
    Private Sub mode(sender As Object, e As EventArgs) Handles f_mode.Click, t_mode.Click, obs_mode.Click
        ...
        If Tag Is "Indirect Fire" Then If observer.title Is Nothing Then If sender.name = "obs_mode" Then Exit Sub
    ...
    Private Sub return_fire_available()
        ...
        If Tag Is "Air to Air" Then Exit Sub
    ...
    Private Sub swap_Click(sender As Object, e As EventArgs) Handles swap.Click
        ...
        If Tag Is "Direct Fire" Then
            direct_fire_phase(ph, nph)
        ElseIf Tag Is "Smoke Barrage" Then
            smoke_barrage_phase(ph)
        ElseIf Tag Is "Indirect Fire" Then
    ...
    Private Sub ta_ecm_gs_Click(sender As Object, e As EventArgs) Handles ta_ecm_gs.Click, ta_ecm_ds.Click, ta_close_escort.Click
        ...
        Dim support = New Collection
        If Mid(interceptor, 10) = p1 Then support = p2_air Else support = p1_air
        For Each ac As cunit In support
    ...
    Private Sub eligible_to_fire(sender As Object)
        ...
        End If
        'Or (Tag is "Air to Ground" And Not firer.heli And rge > 1000)
        ...
        ElseIf range_not_needed Then
            ...
            fire.Enabled = True
            Exit Sub
        ElseIf Tag Is "Direct Fire" Or Tag Is "Half Fire" Or (Tag Is "Opportunity Fire" And Not target.heli) Or (Tag Is "Air to Ground" And firer.task <> "PGM") Then
            If firers.Items.Count = 0 Or targets.Items.Count = 0 Then Exit Sub
            ...
            If Tag Is "Direct Fire" And fire.Enabled Then
                If Not spotting(rge, target, firer) Then return_fire_disable()
        ...
        Else
        End If
        If firer.firers = 0 Or target.title Is Nothing Or firer.title Is Nothing Or tgt_range.ForeColor = Color.Red Or (target.firers = 0 And Tag Is "Direct Fire" And return_fire.BackColor = golden) Then return_fire.Enabled = False
    End Sub
    ...
    Private Sub fire_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles fire.Click
        ...
        If Tag Is "Indirect Fire" And visrange.Text = "Vis Range" Then Exit Sub
        If target.title Is Nothing Or targets.Items.Count = 0 Then Exit Sub
        ac_firer = New cunit
        If Tag Is "Air to Ground" And firer.second_attack And Not firer.disrupted_gt Then
            ...
            With t
                .Text = target.title
                .SubItems.Add(Str(target.strength))
        ...
        For i As Integer = stage To stages
            If area_fire Then
                For Each l As ListViewItem In targets.Items
                    If l.BackColor = golden And l.Tag Is "Area" Then target = enemy(l.Text) : Exit For
                Next
            ...
            If firer.heli And (Tag Is "Direct Fire" Or InStr(Text, "Moving Fire") > 0) Then
                ...
                reset_strength(s1, s2, s3)
                reset_target()
            ElseIf Tag Is "Opportunity Fire" Then
                reset_strength(s1, s2, s3)
                reset_unit_options(directfirepanel)
                reset_range()
                firers.FindItemWithText(firer.title).BackColor = nostatus
    ...
    Private Sub combat_Closed(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.Closing
        ...
        ElseIf Tag Is "Ground to Air" And ground_air_required(False) Then
            ...
            Else
            End If
        ElseIf Tag Is "Direct Fire" Then
    ...
    Private Sub reset_firer()
        If firer.firers_available <= 0 Or Tag Is "Indirect Fire" Then
            If Tag Is "Indirect Fire" And scoot.BackColor = golden Then
            ...
            If firer.firers_available <= 0 Then
                ...
                End If
            Else
                If Tag Is "Indirect Fire" Then artillery.Items(nf).Selected = False Else firers.Items(nf).Selected = False
            End If
            If Tag Is "Indirect Fire" Then
                reset_strength(a1, a2, a3)
            Else
            ...
            End If
        Else
            If Tag Is "Indirect Fire" Then firer_strength(a1, a2, a3, firer.firers_available, False) Else firer_strength(s1, s2, s3, firer.firers_available, firer.airborne)
        End If
    End Sub
    Private Sub reset_aircraft(airunit As cunit, f As Boolean, stage As Integer, tgt_destroyed As Boolean)
        ...
        If (stage = 3 And airunit.airborne) Or tgt_destroyed Or (stage = 1 And airunit.airborne And airunit.scoot) Then
            ...
            Else
                ...
                'If Tag is "Air to Air" Then targets.FindItemWithText(airunit.title).SubItems(3).Text = 4 - airunit.tacticalpts
            End If
            reset_strength(s1, s2, s3)
            reset_strength(t1, t2, t3)
            return_fire_disable()
